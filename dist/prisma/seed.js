...
async function seedTransactions(users) {
    ...
    const profit1 = await prisma.transaction.create({
        data: {
            userId: users[1].id,
            type: enums_1.TransactionType.PROFIT,
            status: enums_1.TransactionStatus.COMPLETED,
            amount: 27.4,
            asset: enums_1.CryptoAsset.USD,
    ...
    const withdrawal1 = await prisma.transaction.create({
        data: {
            userId: users[2].id,
            type: enums_1.TransactionType.WITHDRAWAL,
    ...
    const referralBonus = await prisma.transaction.create({
        data: {
            userId: users[1].id,
            type: enums_1.TransactionType.REFERRAL_BONUS,
            status: enums_1.TransactionStatus.COMPLETED,
            amount: 50,
            asset: enums_1.CryptoAsset.USD,
...
async function seedEvents() {
    ...
    const event2 = await prisma.event.create({
        data: {
            ...
            endDate: new Date(futureDate2.getTime() + 1 * 60 * 60 * 1000),
            isActive: true,
        },
    });
    const event3 = await prisma.event.create({
        data: {
            ...
            endDate: new Date(futureDate3.getTime() + 8 * 60 * 60 * 1000),
            isActive: true,
        },
    });
...
async function seedAchievements(users) {
    ...
    const achievement1 = await prisma.achievement.create({
        data: {
            ...
            type: enums_1.AchievementType.INVESTMENT_COUNT,
            targetValue: 1,
            currentValue: 2,
            isComplete: true,
    ...
    const achievement3 = await prisma.achievement.create({
        data: {
            ...
            type: enums_1.AchievementType.INVESTMENT_AMOUNT,
            targetValue: 50000,
            currentValue: 12000,
            isComplete: false,
        },
    ...
    const achievement5 = await prisma.achievement.create({
        data: {
            ...
            type: enums_1.AchievementType.DEPOSIT_AMOUNT,
            targetValue: 25000,
            currentValue: 10000,
            isComplete: false,
        },
